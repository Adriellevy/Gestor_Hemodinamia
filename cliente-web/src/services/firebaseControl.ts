...
const APP_ID = 'Gestor_Hemodinamia';
...
export async function sendBiSnapshotNow(): Promise<void> {
  try {
    ...
    studies.forEach((s: any) => {
      const mod = s.modalidad || 'Hemodinamia';
    ...
    console.log('📈 [Firebase BI Hemodinamia] Snapshot guardado con éxito:', {
      pedidosCreados: studies.length,
      usuariosIniciadosSesion: user ? 1 : 0,
      whatsappsIntentados: localWhatsappCount,
    });
  } catch (err) {
    console.error('❌ [Firebase BI Hemodinamia] Error al guardar snapshot en Firestore (Verificar reglas de Firebase):', err);
  }
}
...
export function subscribeToAppStatus(onStatusChange: (enabled: boolean) => void): () => void {
  ...
  const unsubscribe = onSnapshot(
    ...
    (err) => {
      console.error('Error suscribiendo al estado de Gestor_Hemodinamia:', err);
    }
  );
...
export async function logStudyCreated(metadata: { modalidad?: string; prioridad?: string; sector?: string; estado?: string }): Promise<void> {
  try {
    await addDoc(collection(db, 'study_creation_events'), {
      appId: APP_ID,
      timestamp: serverTimestamp(),
      modalidad: metadata.modalidad || 'Hemodinamia',
      prioridad: metadata.prioridad || 'normal',
      sector: metadata.sector || 'General',
      estado: metadata.estado || 'solicitado',
    });
    // Actualizar snapshot de BI en tiempo real
    sendBiSnapshotNow();
  } catch (err) {
    console.error('Error enviando evento de creación de estudio en Hemodinamia:', err);
  }
}
...
export async function logWhatsAppAttempt(metadata: { tipo: string; studyId?: string; usuario?: string }): Promise<void> {
  try {
    ...
    await addDoc(collection(db, 'app_whatsapp_events'), {
      ...
      usuario: metadata.usuario || 'Usuario Hemodinamia',
    });
    // Actualizar snapshot de BI en tiempo real
    sendBiSnapshotNow();
  } catch (err) {
    console.error('Error registrando intento de WhatsApp en Hemodinamia:', err);
  }
}
...
export async function logUserSession(usuario: { id?: string; nombre?: string; rol?: string }): Promise<void> {
  try {
    await addDoc(collection(db, 'app_user_sessions'), {
      ...
      nombre: usuario.nombre || 'Usuario Hemodinamia',
      rol: usuario.rol || 'clínico',
    });
    sendBiSnapshotNow();
  } catch (err) {
    console.error('Error registrando sesión de usuario en Hemodinamia:', err);
  }
}
  ...
  const sendHeartbeatAndBi = async () => {
    try {
      await addDoc(collection(db, 'app_usage_events'), {
        ...
        timestamp: serverTimestamp(),
        intervalSeconds: 300,
        clientType: 'docker-hemodinamia',
      });
      await sendBiSnapshotNow();
    } catch (err) {
      console.error('Error enviando telemetría de Hemodinamia:', err);
    }
  };

  // Esperar 4 segundos al inicio para asegurar que fetchData() cargó los pedidos en el store
