    ...
    <>
      ...
      <div className="pl-4 pr-3 py-3">
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          {study.aislamiento && (
            <p className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs font-medium bg-red-50 text-red-700 border border-red-200">
              <ShieldAlert size={11} /> Aislamiento de contacto
            </p>
          )}
          {study.recetaDigitalUrl && (
            <p className="inline-flex items-center">
              <button onClick={() => setVerArchivo(true)} className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors">
                Ver Receta Digital
              </button>
            </p>
          )}
      ...
      {verArchivo && study.recetaDigitalUrl && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 sm:p-6 backdrop-blur-sm" style={{ animation: "fade .2s ease" }} onWheel={(e) => e.stopPropagation()} onClick={cerrarModal}>
          <div className="relative flex h-full w-full flex-col overflow-hidden rounded-xl bg-white shadow-2xl" style={{ animation: "pop .2s ease" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <h3 className="font-medium text-slate-800">Archivo del estudio</h3>
              <div className="flex items-center gap-2">
                {study.recetaDigitalUrl.match(/\.(jpeg|jpg|gif|png)$/i) && (
                  <div className="flex items-center gap-1 rounded-lg bg-slate-100 p-0.5 mr-2">
                    <button onClick={() => setZoom(z => Math.max(0.2, z - 0.2))} className="grid h-6 w-6 place-items-center rounded bg-white text-slate-600 shadow-sm hover:bg-slate-50" title="Alejar">-</button>
