...
import { PROCEDIMIENTOS, PRIORITIES, STATUS } from '../../utils/constants';
...
export function BoardView({ studies, now, onExit }: BoardViewProps) {
  ...
  const ESTADO_BOARD: Record<string, any> = {
  ...
  PROCEDIMIENTOS.forEach((t) => {
    if (t.id === "valvulas" || t.id === "endoprotesis") {
      if (!vistos.has("estructural")) { chips.push({ id: "estructural", short: "Válvulas/Endopr.", Icon: t.Icon, match: ["valvulas", "endoprotesis"] }); vistos.add("estructural"); }
    } else {
