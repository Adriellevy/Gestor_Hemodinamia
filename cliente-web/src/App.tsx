...
import { PROCEDIMIENTOS, SECTORES, PRIORITIES, ROLES, TRASLADOS, STATUS } from "./utils/constants";
...
export default function App() {
  ...
  // Suscribirse a Firebase para flag de bloqueo y telemetría de 5 minutos
  useEffect(() => {
  ...
  // Add & Update logic
  const handleAddStudy = async (data: any, file?: File) => {
    ...
    const newPedido = await createPedido({
      ...
      modalidad: data.modalidad, descripcion: data.descripcion.trim(), conContraste: data.conContraste, aislamiento: data.aislamiento, prioridad: data.prioridad,
      motivo: data.motivo.trim(), tipoTraslado: data.tipoTraslado,
    ...
    if (newPedido && file) {
      try {
        ...
        if (res.ok) {
          ...
          await updatePedido(newPedido.id, { recetaDigitalUrl: updated.recetaDigitalUrl });
        }
      } catch (err) {
  ...
  const handleUpdateStudy = (id: string, d: any, file?: File) => {
    ...
    updatePedido(id, {
      ...
      if (file) {
        try {
          ...
          if (res.ok) {
            const updated = await res.json();
            await updatePedido(id, { recetaDigitalUrl: updated.recetaDigitalUrl });
          }
        } catch (err) {
          console.error('Error uploading recipe:', err);
        }
